...
drop_recovery_plot <- ggplot(plot_data, aes(x = Agency, y = Percent_Change, fill = Metric)) +
...
![Drop and Recovery by Agency](images/drop_recovery_plot.png){fig-align="center" width="80%"}
...
#Creating ggplot
remote_zip_plot <- ggplot(zip_remote_mode, aes(x = reorder(ZIP, Remote_Work_Pct), y = Remote_Work_Pct, fill = Likely_Mode)) +
  geom_col() +
...
![Remote Work Rates by ZIP and Ridership Loss](images/remote_work_zip.png){fig-align="center" width="85%"}
...
remote_vs_ridership_plot <- ggplot(zip_remote_mode, aes(x = Remote_Work_Pct, y = Pct_Change, color = Likely_Mode)) +
  geom_point(size = 4) +
...
![Relationship Between Remote Work and Transit Ridership Change](images/remote_vs_ridership.png){fig-align="center" width="80%"}
