...
# Boxplot comparison
ggplot(combined_data, aes(x = explicit_label, y = popularity.x, fill = explicit_label)) +
...
ggplot(combined_data, aes(x = popularity_group, y = valence.x, fill = popularity_group)) +
...
```{r}
# Optional: Songs with similar valence (positivity)
...
Step - Slice top 20 (based on popularity or appearances — your call!)
...
ggplot(ultimate_playlist, aes(x = factor(track_order), y = valence, group = 1)) +
...
Step4 (Description about the playlist)

**Title**: *Neon Nights: A Vibe-Driven Ride*\
**Description**: A playlist that flows like a nighttime city drive — starting mellow, building in tempo and energy, and fading into reflective tones.\
**Design Principles**:
