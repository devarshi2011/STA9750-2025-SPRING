...
With the data cleaned and ready to groove, this task dives into the world of playlists and track stats to answer a series of focused questions. From identifying chart-toppers and dance floor magnets to spotting missing track data and uncovering playlist trends, this exploration transforms raw data into rhythmic revelations.

### 1️⃣ How many distinct tracks and artists are represented in the playlist data?
...
### 2️⃣ What are the 5 most popular tracks in the playlist data?
...
### 3️⃣ What is the most popular track in the playlist data that does not have a corresponding entry in the song characteristics data?
...
### 4️⃣ According to the song characteristics data, what is the most “danceable” track? How often does it appear in a playlist?
...
### 5️⃣ Which playlist has the longest average track length?
...
### 6️⃣ What is the most popular playlist on Spotify?
...
In "Visualizing the Vibe," the focus is on using clear and effective visualizations to explore the characteristics of popular songs. Through carefully chosen charts, this task answers key questions about the data, with each step explained in a straightforward manner. Key findings are highlighted, ensuring they are placed in context for easy understanding and insightful interpretation. The goal is to present the data in a way that is both informative and visually appealing, making complex patterns accessible without overwhelming the reader
...
### 1️⃣ Is the popularity column correlated with the number of playlist appearances? If so, to what degree?
...
### 2️⃣ In what year were the most popular songs released?
...
### 3️⃣ In what year did danceability peak?
...
### 4️⃣ Which decade is most represented on user playlists?
...
### 6️⃣ Polar (circular) coordinates

Polar (circular) coordinates to visualize the frequency of musical keys across songs
...
### 7️⃣ What are the most popular track lengths? (Are short tracks, long tracks, or something in between most commonly included in user playlists
...
#### ➡️ Are explicit songs more or less popular than non-explicit songs?
...
#### ➡️ Do popular songs tend to be more “positive” in valence?
...
➡️ Finding songs on the same playlist as Anchor songs
...
➡️ Finding songs with similar key and tempo
...
➡️ Finding songs by the same artist
...
➡️ Finding songs with similar Release year, Acousticness and Danceability
...
➡️ Find songs with similar Valence (Custom Heuristic)
...
➡️ Collecting matches from all Heuristics
...
➡️ Select the final 20 songs with Popularity filter
...
```{r}
playlist_title <- "Echoes of Euphoria"
```
