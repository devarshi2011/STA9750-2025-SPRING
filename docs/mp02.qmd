...
```{r}
# Find the state with the highest electricity price
library(dplyr) 
library(scales)  
library(DT)  
...
# Find the state with the 'dirtiest' electricity mix
...
```{r}
# Calculate the weighted average of CO₂ emissions per MWh
library(dplyr)  
library(scales)  
library(DT)      
...
# Count occurrences of each primary energy source
...
```{r}
# Filter data for New York and Texas
library(dplyr)
library(scales)
library(DT)
...
NTD_ENERGY <- NTD_ENERGY |> 
    mutate(Mode = case_when(
        ...
        Mode == "SR" ~ "Streetcar",
        Mode == "F"  ~ "Ferry",
        TRUE ~ "Unknown" # Assign "Unknown" to unidentified modes
    ))
```
...
### **Exploring the NTD Service Data**
...
# Find the transit service with the most annual UPT
...
# Calculate the average trip length for MTA in NYC
...
#### Calculating Total Emissions by joining tables
...
# Ensure `Mode` exists in `NTD_SERVICE` before joining
...
# Filter for agencies with zero emissions
...
# Get top 10 agencies with highest CO2 emissions
...
# Summarize CO2 emissions by transport mode
...
# Compute agency-level total UPT
...
# Find the most efficient agency per category
...
NTD_JOINED <- NTD_JOINED %>%
    mutate(
        Car_Emissions = (MILES / cafe_mpg) * gasoline_co2_per_gallon,  # Emissions if all passengers drove
        Emissions_Avoided = Car_Emissions - Total_CO2_Emissions  # Emissions avoided by transit
    )
...
#### Most Multi-Modal Agency

```{r}
library(dplyr)
library(DT)

# Count how many distinct modes each agency operates
...
# Display the top multi-modal agency
datatable(
  multi_modal,
  options = list(pageLength = 5, autoWidth = TRUE),
...
#### Least Efficient Transit Agency
...
# Wrap long agency names so they don't overflow
...
ggplot(worst_10, aes(x = Emissions_per_UPT, y = reorder(Agency, Emissions_per_UPT))) +
  ...
  theme(
    axis.text.y = element_text(size = 7),  # Shrinks y-axis label size
